...
const CartItem = ({ product, removeFromCart }: { product: AssetFeedItem, removeFromCart: (id: number) => void }) => {
  return (
    <div style={{ display: 'flex', gap: '16px', alignItems: 'center', padding: '16px', backgroundColor: 'var(--bg-navbar)', borderRadius: 'var(--radius-card)', border: '3px solid var(--bg-card-border)' }}>
      <div style={{ width: '80px', height: '80px', borderRadius: 'var(--radius-sm)', backgroundColor: '#d4d0cc', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '40px', flexShrink: 0, position: 'relative', overflow: 'hidden' }}>
        {product.cover_picture ? (
          <img src={product.cover_picture} alt={product.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        ) : (
          '🚂'
        )}
      </div>
      <div style={{ flex: 1 }}>
        <h3 style={{ fontFamily: 'var(--font-body)', fontWeight: 800, fontSize: '16px', color: 'var(--color-text-primary)' }}>{product.name}</h3>
        <p style={{ fontFamily: 'var(--font-pixel)', fontSize: '12px', color: 'var(--color-text-secondary)', marginTop: '4px' }}>
...
export default function CartPage() {
  const { cart, removeFromCart, clearCart, userId } = useAuth();
  const router = useRouter();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [cartProducts, setCartProducts] = useState<AssetFeedItem[]>([]);
  ...
  useEffect(() => {
    const fetchCartProducts = async () => {
      ...
      try {
        setIsLoading(true);
        // Fetch all products and filter them based on the cart IDs
        const response = await getAssetFeed(userId);
        const productsInCart = response.items.filter(item => cart.includes(item.id));
  ...
  const subtotal = cartProducts.reduce((sum, product) => sum + parseFloat(product.price), 0);
  ...
  const renderContent = () => {
    if (isLoading) {
      return <p style={{ textAlign: 'center', fontFamily: 'var(--font-body)', color: 'var(--color-text-secondary)' }}>Carregando itens do carrinho...</p>;
    }
